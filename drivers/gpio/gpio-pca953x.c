...
static ssize_t store_set_dir(struct device *dev,struct device_attribute *attr, const char *buf, size_t len)
{
	u8 val[MAX_BANK] = {0};
	int ret;
	struct pca953x_chip *chip = dev_get_drvdata(dev);
...
static ssize_t store_set_val(struct device *dev,struct device_attribute *attr, const char *buf, size_t len)
{
	u8 val[MAX_BANK] = {0};
	int ret;
	struct pca953x_chip *chip = dev_get_drvdata(dev);
