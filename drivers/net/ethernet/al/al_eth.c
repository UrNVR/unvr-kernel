...
static int al_mod_eth_board_params_of_init(struct al_mod_eth_adapter *adapter, struct al_mod_eth_board_params* params) {
	...
	np = of_find_matching_node(NULL, al_mod_eth_of_table);
	if (!np) {
	...
	np = of_find_node_by_name(np, path);
	if (!np) {
	...
	/* LED setup */
	al_mod_eth_board_of_led(adapter, params, np);

	/* LM SFP probing */
	al_mod_eth_board_of_sfp_probing(adapter, params, np);
...
#if defined(CONFIG_ARCH_ALPINE)
static int al_mod_eth_i2c_data_read(void *context,
				uint8_t bus_id,
				uint8_t i2c_addr,
				uint8_t reg_addr,
				uint8_t *val,
...
static int al_mod_eth_i2c_data_write(void *context,
				 uint8_t bus_id,
				 uint8_t i2c_addr,
				 uint8_t reg_addr,
				 uint8_t *val,
...
static int al_mod_eth_i2c_byte_read(void *context,
				uint8_t bus_id,
				uint8_t i2c_addr,
				uint8_t reg_addr,
				uint8_t *val)
{
	...
	return al_mod_eth_i2c_data_read(context, bus_id, i2c_addr, reg_addr, val, 1);
}

static int al_mod_eth_i2c_byte_write(void *context,
				 uint8_t bus_id,
				 uint8_t i2c_addr,
				 uint8_t reg_addr,
				 uint8_t val)
{
	return al_mod_eth_i2c_data_write(context, bus_id, i2c_addr, reg_addr, &val, 1);
}
...
static int al_mod_eth_v4_lm_init(struct al_mod_eth_adapter *adapter, bool first_time)
{
	 ...
	 * once we will want to run this al_eth on K2 that runs linux - we will need
...
int al_mod_eth_default_rx_udma_num_set(struct al_mod_eth_adapter *adapter, unsigned int rx_udma_num)
{
	 ...
	 *
	 * (*) FLR - FLR will be performed only for the udma which is controlled by al_eth
	 ...
	 * will reach the al_eth port MAC and will effect the adapter->udma_num udma.
	 * That means that if the adapter->default_rx_udma is starting to backpressure due to
	 * traffic directed to it from al_eth it will send flow control packet (Xoff)
