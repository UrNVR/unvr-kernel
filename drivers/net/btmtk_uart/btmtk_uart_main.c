		...
		const int cmd_len, int retry, int endpoint, unsigned long tx_state)
{
	...
	BTMTK_DBG_RAW(cmd, cmd_len, "%s, len = %d Send CMD : ",
				__func__, cmd_len);
		...
		const int event_len, int retry, int endpoint, unsigned long tx_state)
{
	...
	if (event_len > 0) {
		BTMTK_DBG_RAW(event, event_len, "%s, len = %d Recv CMD : ",
				__func__, event_len);
		/* Compare with skb_evt. */
		skb = bdev->evt_skb;
		if (skb) {
			if (!memcmp(skb->data, event, event_len)) {
				BTMTK_DBG("Get event result: OK\n");
			} else {
				BTMTK_INFO("Get event result: NG\n");
				BTMTK_INFO_RAW(skb->data, event_len,
					"%s, len = %d Recv CMD : ", __func__, event_len);
				return -1;
			}
			return skb->data[6];
		} else {
			BTMTK_INFO("skb is NULL\n");
			return -1;
		}
	} else {
		return 1;
			      ...
			      unsigned int cmd, unsigned long arg)
{
	...
	case HCIUARTSETBAUD:
		...
		btmtk_uart_send_set_uart_cmd(g_bdev->hdev, arg);
		msleep(100);
		return 1;
	...
	case HCIUARTSETSTP:
		...
		btmtk_send_set_stp_cmd(g_bdev->hdev);
		btmtk_send_set_stp1_cmd(g_bdev->hdev);
		return 1;
	case HCIUARTSTPENABLE:
		pr_info("<!!> Set STP enable <!!>\n");
		mtk_stp_enable();
		return 1;
	case HCIUARTLOADPATCH:
		...
		return 1;
	case HCIUARTLOADFLASH:
#if LOAD_ROM_PATCH_TO_FLASH
		pr_info("<!!> Set HCIUARTLOADFLASH command <!!>\n");
		btmtk_load_flash_programing(g_bdev->hdev);
#else
		pr_info("<!!> HCIUARTLOADFLASH not support <!!>\n");
#endif
		return 1;
	default:
