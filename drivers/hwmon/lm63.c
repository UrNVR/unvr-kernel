...
static void lm63_init_client(struct lm63_data *data)
{
	...
	if (client->dev.of_node) {
		...
		if (!of_property_read_u32(client->dev.of_node, "temp2-crit", &temp2_crit)) {
			data->config |= 0x02;
			i2c_smbus_write_byte_data(client, LM63_REG_CONFIG1,
				data->config);
			i2c_smbus_write_byte_data(client, LM63_REG_REMOTE_TCRIT,
				(temp2_crit >= 0) ? TEMP8U_TO_REG(temp2_crit): TEMP8_TO_REG(temp2_crit));
		}
	}
		      ...
		      const struct i2c_device_id *id)
{
	...
	if (client->dev.of_node) {
		...
		if (!of_property_read_u32(client->dev.of_node, "pwm-freq", &pwm_freq))
